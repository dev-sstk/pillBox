        ...
        try:
            if hasattr(self.screen_manager, 'screens') and 'dose_time' in self.screen_manager.screens:
                ...
                if hasattr(dose_time_screen, 'get_dose_times'):
                    ...
                    print(f"  📱 dose_time_screen 메서드 없음, 기본값 사용")
            else:
                ...
                print(f"  📱 dose_time_screen 없음, 기본값 사용")
        except Exception as e:
            # 오류 시 기본값 사용
            self.dose_schedule = [
                {"time": "08:00", "status": "pending"},
    ...
    def on_button_c(self):
        ...
        try:
            ...
            if disk_success:
                ...
                # 2. 배출구 열림 (슬라이드 1단계)
                print(f"  🔧 배출구 열림 (슬라이드 1단계)")
                self._update_status("배출구 열림 중...")
                open_success = motor_system.control_dispense_slide(1)
                ...
                if open_success:
                    ...
                    # 3. 배출구 닫힘 (같은 각도로 역회전)
                    print(f"  🔧 배출구 닫힘 (슬라이드 1단계 역회전)")
                    self._update_status("배출구 닫힘 중...")
                    close_success = motor_system.control_dispense_slide_close(1)
