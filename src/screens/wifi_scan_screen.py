...
class WifiScanScreen:
    ...
    def _create_modern_screen(self):
        ...
        print(f"  [INFO] 화면 크기: {screen_w}x{screen_h}")
        ...
        if not hasattr(self, 'main_container') or self.main_container.get_width() == 0:
            print(f"  [WARN] 메인 컨테이너 문제로 화면에 직접 생성합니다.")
            ...
            print(f"  [INFO] Wi-Fi 리스트 영역 생성 완료")
    ...
    def _create_main_container(self):
        ...
        print(f"    [INFO] 메인 컨테이너 위치: ({main_x}, {main_y}), 크기: {main_w}x{main_h}")
        ...
        if main_w == 0 or main_h == 0:
            ...
            main_w = self.main_container.get_width()
            main_h = self.main_container.get_height()
            print(f"    [INFO] 재설정 후 메인 컨테이너 크기: {main_w}x{main_h}")
    
    def _create_direct_screen_elements(self):
        """화면에 직접 요소들 생성 (메인 컨테이너 문제 시)"""
        print(f"  [INFO] 화면에 직접 요소들 생성 시작...")
        ...
        if hasattr(lv, "font_notosans_kr_regular"):
            ...
            print(f"  [INFO] 제목 텍스트 생성 완료 (한글 폰트 사용)")
        else:
            # 폰트를 설정하지 않음 (기본값 사용)
            print(f"  [INFO] 제목 텍스트 생성 완료 (기본 폰트 사용)")
        ...
        # 스캔 상태 아이콘 제거
        print(f"  [INFO] 스캔 상태 아이콘 제거됨")
        
        # 모던 WiFi 리스트 생성
        self._create_modern_wifi_list()
        print(f"  [INFO] 모던 WiFi 리스트 생성 완료")
        ...
        print(f"  [INFO] 버튼 힌트 생성 완료 (기본 폰트 사용)")
    ...
    def _create_modern_wifi_list(self):
        ...
        print(f"  [INFO] LVGL 리스트 위젯 WiFi 리스트 생성 시작...")
        ...
        try:
            ...
            print(f"  [INFO] LVGL 리스트 위젯 생성 완료")
            
            # 최대 4개 네트워크만 표시
            max_networks = min(len(self.wifi_networks), 4)
            print(f"  [INFO] {max_networks}개 네트워크 리스트 아이템 생성 예정...")
            ...
            for i in range(max_networks):
                try:
                    ...
                    print(f"  [OK] 네트워크 {i+1} 리스트 아이템 생성 완료")
                    
                except Exception as e:
                    print(f"  [ERROR] 네트워크 {i+1} 리스트 아이템 생성 실패: {e}")
                    import sys
            ...
            if self.wifi_list_items:
                try:
                    ...
                    print(f"  [INFO] 첫 번째 아이템 선택됨")
                except AttributeError:
                    # focus 메서드가 없으면 다른 방법 시도
                    try:
                        self.wifi_list.set_focused(self.wifi_list_items[0])
                        print(f"  [INFO] 첫 번째 아이템 선택됨 (set_focused)")
                    except AttributeError:
                        print(f"  [INFO] 첫 번째 아이템 선택 건너뜀 (메서드 없음)")
            
            print(f"  [OK] {len(self.wifi_list_items)}개 LVGL 리스트 WiFi 네트워크 생성 완료")
            
        except Exception as e:
            print(f"  [ERROR] LVGL 리스트 WiFi 생성 실패: {e}")
            import sys
            sys.print_exception(e)
            self.wifi_list_items = []
    ...
    def _create_wifi_list(self):
        ...
        print(f"  [INFO] Wi-Fi 네트워크 리스트 생성...")
    ...
    def _update_selection(self):
        ...
        try:
            if not hasattr(self, 'wifi_labels') or not self.wifi_labels:
                print("  [WARN] WiFi 아이템이 없어서 선택 업데이트 건너뜀")
                return
                
            print(f"  [INFO] 선택 업데이트: {len(self.wifi_labels)}개 아이템, 선택된 인덱스: {self.selected_index}")
                ...
                try:
                    if i == self.selected_index:
                        ...
                        try:
                            ...
                            pass
                        print(f"    [OK] 아이템 {i+1} 선택됨 (모던 하이라이트)")
                    else:
                        # 일반 항목 스타일 (모던 카드)
                        wifi_item.set_style_bg_color(lv.color_hex(0xFFFFFF), 0)  # 흰색 배경
                        wifi_item.set_style_bg_opa(255, 0)
                        ...
                        wifi_item.set_style_shadow_ofs_x(0, 0)
                        wifi_item.set_style_shadow_ofs_y(2, 0)
                        print(f"    [OK] 아이템 {i+1} 일반 상태 (모던 카드)")
                        
                except Exception as e:
                    print(f"    [ERROR] 아이템 {i+1} 스타일 설정 실패: {e}")
                    continue
    ...
    def show(self):
        ...
        if self.screen_obj:
            ...
            try:
                ...
                print(f"[ERROR] {self.screen_name} 화면 로드 실패: {e}")
                return
            
            # 로드 후 활성 화면 확인 (안전한 방식)
            try:
                new_screen = lv.screen_active()
                print(f"[INFO] 로드 후 활성 화면: {new_screen}")
            except Exception as e:
                print(f"[WARN] 로드 후 활성 화면 확인 실패: {e}")
    ...
    def on_button_a(self):
        ...
        # 단계별 디버깅
        print(f"  [INFO] wifi_list_items 존재: {hasattr(self, 'wifi_list_items')}")
        if hasattr(self, 'wifi_list_items'):
            print(f"  [INFO] wifi_list_items 값: {self.wifi_list_items}")
        print(f"  [INFO] current_selected_index 존재: {hasattr(self, 'current_selected_index')}")
        if hasattr(self, 'current_selected_index'):
            print(f"  [INFO] current_selected_index 값: {self.current_selected_index}")
        
        if hasattr(self, 'wifi_list_items') and self.wifi_list_items and hasattr(self, 'current_selected_index'):
            print(f"  [INFO] 조건 통과 - 네트워크 선택 시작")
            
            # 현재 선택된 인덱스로 네트워크 선택
            selected_index = self.current_selected_index
            print(f"  [INFO] 선택된 인덱스: {selected_index}")
            print(f"  [INFO] wifi_networks 길이: {len(self.wifi_networks)}")
            
            if 0 <= selected_index < len(self.wifi_networks):
                print(f"  [INFO] 인덱스 범위 확인 통과")
                selected_network = self.wifi_networks[selected_index]
                print(f"  [INFO] 선택된 네트워크: {selected_network['ssid']}")
                print(f"  [INFO] 보안 타입: {selected_network.get('security', 'Unknown')}")
                ...
                if any(sec_type in security for sec_type in ['wpa2', 'wpa3', 'wep', 'wpa', 'psk', 'sae']):
                    ...
                    if saved_password is not None:
                        # 저장된 비밀번호가 있으면 자동 연결
                        print(f"  [SAVE] 저장된 비밀번호 사용하여 자동 연결 시도: {selected_network['ssid']}")
                        success = get_wifi_manager().connect_to_network(selected_network['ssid'], saved_password)
                        
                        if success:
                            print(f"  [OK] 저장된 비밀번호로 연결 성공!")
                            time.sleep(1)
                            # 저장된 비밀번호로 연결 성공 시 바로 다음 화면으로 이동
                            self._go_to_next_screen()
                        else:
                            print(f"  [ERROR] 저장된 비밀번호로 연결 실패! 비밀번호 입력 화면으로 이동")
                            # 저장된 비밀번호가 틀렸으면 비밀번호 입력 화면으로
                            self._show_password_screen(selected_network)
                        return
                    
                    # 저장된 비밀번호가 없으면 패스워드 화면으로 이동
                    print(f"  [INFO] 저장된 비밀번호 없음, 패스워드 화면으로 이동")
                    self._show_password_screen(selected_network)
                else:
                    # 보안이 없는 네트워크 - 직접 연결 시도
                    print(f"  🔓 오픈 네트워크: {security}")
                    print(f"  [INFO] 오픈 네트워크 연결 시도...")
                    self._connect_to_open_network(selected_network)
                    print(f"  [OK] 오픈 네트워크 연결 완료")
            else:
                print(f"  [INFO] 잘못된 선택 인덱스: {selected_index}")
        else:
            print(f"  [ERROR] 조건 불만족 - wifi_list_items 또는 current_selected_index 없음")
    ...
    def on_button_b(self):
        ...
        if hasattr(self, 'wifi_list_items') and self.wifi_list_items:
            ...
            if self.current_selected_index > 0:
                ...
                print(f"  [INFO] 이전 아이템으로 이동: {self.current_selected_index}")
            else:
                print(f"  [INFO] 이미 첫 번째 항목")
            ...
            if self.current_selected_index < len(self.wifi_list_items) - 1:
                ...
                try:
                    ...
                    try:
                        ...
                        # 포커스 설정이 안되면 시각적 하이라이트만
                        self._update_selection_highlight()
                print(f"  [INFO] 다음 아이템으로 이동: {self.current_selected_index}")
            else:
                print(f"  [INFO] 이미 마지막 항목")
    
    def on_button_d(self):
        """버튼 D 처리 - 기능 없음"""
        print("Wi-Fi 스캔 화면 버튼 D - 기능 없음")
        # 버튼 D 기능 제거됨
    
    def _show_password_screen(self, selected_network):
        """비밀번호 입력 화면 표시"""
        print(f"  [INFO] 패스워드 화면 존재 확인: {'wifi_password' in self.screen_manager.screens}")
        print(f"  [INFO] 패스워드 화면 준비 중...")
        
        # 패스워드 화면이 등록되어 있지 않으면 동적 생성
        if 'wifi_password' not in self.screen_manager.screens:
            print(f"  [INFO] 비밀번호 화면 동적 생성 중...")
            ...
            try:
                ...
                print(f"  [OK] 비밀번호 화면 생성 및 등록 완료")
            except Exception as e:
                print(f"  [ERROR] 비밀번호 화면 생성 실패: {e}")
    ...
    def _connect_to_open_network(self, network):
        ...
        if success:
            print("[OK] 오픈 네트워크 연결 성공!")
            time.sleep(1)
            # 다음 화면으로 이동
            self._go_to_next_screen()
        else:
            print("[ERROR] 오픈 네트워크 연결 실패!")
            # 연결 실패 시 현재 화면에 머물기
    ...
    def _scan_wifi_networks(self):
        ...
        try:
            ...
            if scanned_networks:
                ...
                print(f"[OK] {len(self.wifi_networks)}개 네트워크 발견")
            else:
                # 스캔 실패 시 빈 목록
                self.wifi_networks = []
                print("[WARN] WiFi 스캔 결과가 없습니다")
                print("  [TIP] ESP32-C6 WiFi가 제대로 초기화되지 않았을 수 있습니다")
            
            self.scanning = False
            self.last_scan_time = time.ticks_ms()
            
        except Exception as e:
            print(f"[ERROR] WiFi 스캔 오류: {e}")
            import sys
            sys.print_exception(e)
            self.scanning = False
